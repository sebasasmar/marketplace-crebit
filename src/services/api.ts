...
import { sanitizeSearchTerm, isValidUUID, isPositiveNumber } from '@/utils/validators';
...
class ApiService {
  ...
  if (searchTerm) {
    ...
    if (clean) {
      // Use ilike with sanitized, trimmed string
      query = query.or(`nombre.ilike.%${clean}%,cedula.ilike.%${clean}%`);
    }
  }
    if (filters) {
  ...
  */
  ...
  async adminAdjustSaldo(empresaId: string, amount: number): Promise<void> {
    ...
    if (!isValidUUID(empresaId)) throw new Error('Empresa id inválido');
    if (!isPositiveNumber(amount) || amount > 1_000_000_000) throw new Error('Monto inválido');
  ...
  // Validate inputs to avoid accidental bad data being sent to RPC
  if (!isValidUUID(empresaId)) throw new Error('Empresa id inválido');
  if (!isValidUUID(leadId)) throw new Error('Lead id inválido');
  ...
  async createWompiCheckout(amountInCents: number): Promise<{ reference: string; signature: string }> {
    try {
      ...
      // Validate amount
      if (!Number.isInteger(amountInCents) || amountInCents <= 0 || amountInCents > 1_000_000_000) {
        throw new Error('Monto inválido para checkout');
      }
