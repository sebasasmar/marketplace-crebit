...
const Login: React.FC = () => {
    ...
    const renderLogin = () => (
         <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input id="login-email" name="email" type="email" placeholder="tu@empresa.com" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} required className="w-full px-4 py-2 border border-border-light rounded-lg focus:ring-brand-primary focus:border-brand-primary bg-background-light dark:bg-gray-700 dark:border-border-dark" />
            </div>
            <div>
                <label htmlFor="login-password" className="sr-only">Contraseña</label>
                <input id="login-password" name="password" type="password" placeholder="••••••••" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} required className="w-full px-4 py-2 border border-border-light rounded-lg focus:ring-brand-primary focus:border-brand-primary bg-background-light dark:bg-gray-700 dark:border-border-dark" />
            </div>
    ...
    const renderSignup = () => (
         <form onSubmit={handleSignupSubmit} className="space-y-4">
            ...
            <input id="signup-nombre" name="nombreEmpresa" placeholder="Nombre de la Empresa" value={signupData.nombreEmpresa} onChange={e => setSignupData({...signupData, nombreEmpresa: e.target.value})} required className="w-full px-4 py-2 border border-border-light rounded-lg bg-background-light dark:bg-gray-700 dark:border-border-dark" />
            <input id="signup-nit" name="nit" placeholder="NIT" value={signupData.nit} onChange={e => setSignupData({...signupData, nit: e.target.value})} required className="w-full px-4 py-2 border border-border-light rounded-lg bg-background-light dark:bg-gray-700 dark:border-border-dark" />
            <input id="signup-email" name="email" type="email" placeholder="Email de la Empresa" value={signupData.email} onChange={e => setSignupData({...signupData, email: e.target.value})} required className="w-full px-4 py-2 border border-border-light rounded-lg bg-background-light dark:bg-gray-700 dark:border-border-dark" />
            <input id="signup-password" name="password" type="password" placeholder="Crear Contraseña" value={signupData.password} onChange={e => setSignupData({...signupData, password: e.target.value})} required className="w-full px-4 py-2 border border-border-light rounded-lg bg-background-light dark:bg-gray-700 dark:border-border-dark" />
             <div>
                ...
                <select 
                    ...
                    required 
                    className="w-full px-4 py-2 border border-border-light rounded-lg bg-background-light dark:bg-gray-700 dark:border-border-dark"
                >
    ...
    const renderForgot = () => (
         <form onSubmit={handleForgotSubmit} className="space-y-4">
             ...
             <div>
                ...
                <input id="forgot-email" name="email" type="email" placeholder="tu@empresa.com" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} required className="w-full px-4 py-2 border border-border-light rounded-lg bg-background-light dark:bg-gray-700 dark:border-border-dark" />
             </div>
             <button type="submit" disabled={isLoading} className="w-full bg-brand-primary text-white py-2.5 rounded-lg font-semibold hover:bg-brand-primary-dark disabled:bg-opacity-50 flex justify-center">
        ...
        <div className="min-h-screen bg-background-light dark:bg-background-dark flex flex-col justify-center items-center p-4">
            <div className="w-full max-w-md">
                ...
                <div className="bg-card-light dark:bg-card-dark p-8 rounded-2xl shadow-lg">
                    ...
                    {/* Fix: Use the correct Tailwind animation class `animate-fade-in` instead of `fade-in`. */}
